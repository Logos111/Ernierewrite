...
train_path = "./data/chat-ext-train.jsonl"
test_path = "./data/chat-ext-dev.jsonl"
...
dic = {"learning_rate": [], "batch_size": [], "seq_w": [], "cls_w": [], "P": [], "R": [], "F1": [], "ACC": []}
# 嵌套循环遍历所有可能的参数组合
    ...
    for bs in batch_sizes:
        for seq_w in seq_ws:
            for cls_w in cls_ws:
                # try:
                    # 使用当前参数组合进行训练
                    trainer = TrainerForToken(model_name, vocab_path, batch_size=bs, seq_w=seq_w, cls_w=cls_w,
                                              learning_rate=lr, epochs=10, ckpt_dir="ernie_ckpt", 
                                              max_seq_length=1024)
